//! `riffle demo` subcommand — synthetic CDC producer/consumer demo with dashboard.
...
use crate::{consumer, producer, web};

pub async fn run(cfg: Config) -> Result<()> {
    cfg.register_handlers();

    let backend = format!("{:?}", cfg.backend());
    println!("=== riffle demo — Delta Lake CDC Dashboard ===");
...

    {
        ...
        tokio::spawn(async move {
            if let Err(e) = web::run(bind, rx_for_web, tunables_for_web, None, None, None).await {
